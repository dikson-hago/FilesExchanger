...
{
 "use strict";
 var WebSharper,Sitelets,CorsAllows,StringEncoding,PathUtil,Route,List,Router,Error,RouterModule,BodyTextNeededForRoute,Obj,ListArrayConverter,RouterOperators,SC$1,Strings,String,Runtime,List$1,Seq,Collections,Map,Arrays,FSharpMap,Unchecked,Utils,console,Nullable,Numeric,Lazy,Operators,Concurrency,Char,System,Guid,Slice;
 ...
 BodyTextNeededForRoute=RouterModule.BodyTextNeededForRoute=RouterModule.BodyTextNeededForRoute||{};
 ...
 Route.Combine=function(paths)
 {
  ...
  },paths$1),Route.New(List$1.ofSeq(segments),queryArgs,formData,method,body));
 };
 ...
 {
  return Route.New(List$1.T.Empty,new FSharpMap.New([]),new FSharpMap.New([]),null,{
   $:1,
   $0:null
  });
 };
 ...
 BodyTextNeededForRoute=RouterModule.BodyTextNeededForRoute=Runtime.Class({},Error,BodyTextNeededForRoute);
 BodyTextNeededForRoute.New=Runtime.Ctor(function()
 {
  this.message="This router needs body text loaded synchronously. Wait for Http.Request.BodyText to finish then try again.";
  Global.Object.setPrototypeOf(this,BodyTextNeededForRoute.prototype);
 },BodyTextNeededForRoute);
 ...
 RouterModule.FetchWith=function(baseUrl,options,router,endpoint)
 {
  ...
  return m!=null&&m.$==1?(path=m.$0,(options$1.method=(x=path.Method,x==null?"POST":x.$0),m$1=path.Body.$0,m$1===null?!path.FormData.get_IsEmpty()?(fd=new Global.FormData(),Map.Iterate(function(k,v)
  {
 ...
 RouterModule.XHRWith=function(conf,router,endpoint)
 {
  ...
  return m!=null&&m.$==1?(path=m.$0,(conf.ResponseT===void 0?conf.ResponseT="text":void 0,method=(m$1=path.Method,m$1==null?"POST":m$1.$0),Concurrency.FromContinuations(function(ok,err)
  {
   ...
   {
    return err(new Error(xhr.statusText));
   };
   url=path.ToLink();
 ...
 RouterModule.Body=function(deserialize,serialize)
 {
  return Router.New$1(function(path)
  {
   ...
   m$1=path.Body;
   if(m$1!=null&&m$1.$==1)
    return m$1.$0===null?[]:(m=deserialize(m$1.$0),m!=null&&m.$==1?[[Route.New(path.Segments,path.QueryArgs,path.FormData,path.Method,{
     $:1,
     $0:null
    }),m.$0]]:[]);
   else
    throw new BodyTextNeededForRoute.New();
  },function(value)
  {
   var i;
   return{
 ...
 RouterModule.QueryNullable=function(key,item)
 {
  return Router.New$1(function(path)
  {
   ...
   return m!=null&&m.$==1?(newQa=path.QueryArgs.Remove$1(key),Seq.map(function($1)
   {
    return m$1($1[0],$1[1]);
